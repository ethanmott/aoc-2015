...
fn main() {
    let file = File::open("day8.txt").expect("Error opening day8 input file.");
    ...
    println!("day8.1: ({} - {}) = {}", total_chars, total_unescaped_chars, total_chars - total_unescaped_chars);

    let total_escaped_chars: i32 = lines.iter()
        .map(|l| escaped_length(l.as_str()))
        .sum();

    println!("day8.2: ({} - {}) = {}", total_escaped_chars, total_chars, total_escaped_chars - total_chars);
}
